...
- Klanten zijn in NL, België (alleen Vlaanderen) en Duitsland (NRW + Niedersachsen)
- Database: SQL Server op <db-server> (<db-naam>), read-only rechten
- Berichten via: TextMeBot API (WhatsApp, eigen nummer gekoppeld)
- Eigen WhatsApp nummer: <samenvattingsnummer> (ontvangt ook samenvattingsberichten)
...
| Morgen BE/DE én NL feestdag | Eerstvolgende werkdag | Eerstvolgende werkdag | Eerstvolgende werkdag |
...
Haben Sie morgen Kisten bereitstehen aufgrund eines Feiertags? Wenn ja, wie viele Kisten hätten Sie bereit?
...
Aufgrund des niederländischen Feiertags Koningsdag fahren wir morgen nicht. Wissen Sie schon, wie viele Kisten Sie Dienstag (28.04.2026) haben?
...
## Samenvattingsberichten (naar <samenvattingsnummer>)
...
- Overwegen: officiële WhatsApp Business API voor meer betrouwbaarheid op lange termijn
